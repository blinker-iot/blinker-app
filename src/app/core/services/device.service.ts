...
interface V2DeviceBinding {
  device: BlinkerDevice;
  subscriptions: Subscription;
  bridgedKeys: Set<string>;
...
export class DeviceService {
  ...
  private readonly v2Bindings = new Map<string, V2DeviceBinding>();
  ...
  init() {
    this.dataService.initCompleted.subscribe(loaded => {
      if (loaded) {
        ...
        void this.restoreDeviceConfigs();
      }
    })
    this.dataService.authDataExpire.subscribe(state => {
      if (state) {
        this.clearV2Bindings();
  ...
  pubMessage(device, message: string) {
    if (device?.config?.mode === 'bbp2') {
      void this.sendV2Data(device, message).catch((error) => this.handleError(error));
      return;
    }
  ...
  connectDevice(device: any, mode: string = "show") {
    if (device?.config?.mode === 'bbp2') {
      ...
      this.bindV2Device(device, id);
      return this.deviceUi.connect(id)
        .then(() => true)
  ...
  sendData(device, data) {
    if (device?.config?.mode === 'bbp2') {
      void this.sendV2Data(device, data).catch((error) => this.handleError(error));
      return;
    }
  ...
  private logicalDeviceId(device: BlinkerDevice): string {
    return String(device.id || device.deviceName || '').trim();
  }

  private async restoreDeviceConfigs(): Promise<void> {
    const deviceNames = Array.isArray(this.deviceDataList)
      ? [...this.deviceDataList]
      : [];
    await Promise.all(
      deviceNames.map(async (deviceName) => {
        const device = this.deviceDataDict[deviceName] as
          | BlinkerDevice
          | undefined;
        if (!device || device.config.isPreview) return;

        const config = await this.loadDeviceConfig(device);
        if (!this.isRecord(config)) return;

        const previousComponent = device.config.component;
        const transportConfig = {
          mode: device.config.mode,
          broker: device.config.broker,
          disabled: device.config.disabled,
          isShared: device.config.isShared,
          isPreview: device.config.isPreview,
        };
        Object.assign(device.config, config, transportConfig);
        const componentChanged =
          previousComponent !== device.config.component;
        this.notifyDeviceUpdate(
          device,
          componentChanged ? 'component' : 'loaded',
  ...
  private bindV2Device(
    device: BlinkerDevice,
    logicalDeviceId: string,
  ): void {
    const current = this.v2Bindings.get(logicalDeviceId);
    if (current) {
      if (current.device !== device) {
        current.device = device;
        if (current.snapshot) {
          this.applyV2Snapshot(current, current.snapshot);
        }
      }
      return;
    }

    const binding: V2DeviceBinding = {
      device,
      subscriptions: new Subscription(),
      bridgedKeys: new Set<string>(),
    };
    this.v2Bindings.set(logicalDeviceId, binding);
    binding.subscriptions.add(
      this.deviceUi.watchState(logicalDeviceId).subscribe({
        next: (snapshot) => {
          if (this.v2Bindings.get(logicalDeviceId) === binding) {
            this.applyV2Snapshot(binding, snapshot);
          }
        },
    ...
    binding.subscriptions.add(
      this.deviceUi.watchEvents(logicalDeviceId).subscribe({
        next: (event) => {
          if (this.v2Bindings.get(logicalDeviceId) === binding) {
            this.applyV2Event(binding, event);
          }
        },
        error: (error) => this.handleError(error),
      }),
  ...
  private applyV2Snapshot(
    binding: V2DeviceBinding,
    snapshot: DeviceUiSnapshot,
  ): void {
    binding.snapshot = snapshot;
    const data = this.deviceData(binding.device);
    const endpointKeys = new Set(
      snapshot.endpoints.map((endpoint) => endpoint.key),
    );
    for (const key of binding.bridgedKeys) {
      if (!endpointKeys.has(key)) delete data[key];
    }
    binding.bridgedKeys = endpointKeys;

    for (const endpoint of snapshot.endpoints) {
      if (typeof endpoint.value === 'undefined') continue;
      data[endpoint.key] = this.jsonSafeV2Value(endpoint.value);
      if (endpoint.key === 'switch') {
        this.clearSwitchWaitingTimer(binding.device);
      }
    }
    data['state'] = snapshot.stateFresh ? 'online' : 'waiting';
    data['enable'] = snapshot.stateFresh;
    this.notifyDeviceUpdate(
      binding.device,
      'loaded',
      undefined,
  ...
  private applyV2Event(
    ...
    for (const [key, value] of Object.entries(event.values)) {
      ...
      data[key] = this.jsonSafeV2Value(value);
      binding.bridgedKeys.add(key);
      if (key === 'switch') this.clearSwitchWaitingTimer(binding.device);
    }
    this.notifyDeviceUpdate(
      binding.device,
      'loaded',
  ...
  private deviceData(device: BlinkerDevice): Record<string, unknown> {
    if (!this.isRecord(device.data)) device.data = {};
    return device.data;
  }

  private jsonSafeV2Value(value: unknown): unknown {
    if (typeof value === 'bigint') return value.toString();
    if (value instanceof Uint8Array) return Array.from(value);
    if (Array.isArray(value)) {
      return value.map((item) => this.jsonSafeV2Value(item));
    }
    if (this.isRecord(value)) {
      return Object.fromEntries(
        Object.entries(value).map(([key, item]) => [
          key,
          this.jsonSafeV2Value(item),
        ]),
      );
    }
    return value;
  ...
  private clearV2Bindings(): void {
    for (const binding of this.v2Bindings.values()) {
      binding.subscriptions.unsubscribe();
    }
    this.v2Bindings.clear();
  }

  private async sendV2Data(
    device: BlinkerDevice,
    data: unknown,
  ): Promise<void> {
    const logicalDeviceId = this.logicalDeviceId(device);
    if (!logicalDeviceId) throw new Error('设备标识无效');
    this.bindV2Device(device, logicalDeviceId);
